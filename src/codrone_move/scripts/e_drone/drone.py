...
# BaseFunctions Start
    ...
    def __init__(self, flagCheckBackground = True, flagShowErrorMessage = False, flagShowLogMessage = False, flagShowTransferData = False, flagShowReceiveData = False):
        ...
        self.timeStartProgram           = time.time() 
    ...
    def open(self, portname = "None"):
        if eq(portname, "None"):
            ...
            size = len(nodes)
            if size > 0:
                portname = nodes[size - 1].device #portname = /dev/ttyAMA0
        ...
        try:
            ...
            if( self.isOpen() ):
                ...
                self._thread = Thread(target=self._receiving, args=())
                ...
                print("Could not connect to E-DRONE. isOpen=False")
                return False

        except Exception as ex:
                self._printError("Could not connect to E-DRONE.")
                print("Could not connect to E-DRONE. exception interrupt!", ex)
    ...
    def _printReceiveData(self, dataArray):
        ...
        if (self._flagShowReceiveData) and (dataArray != None) and (len(dataArray) > 0):
            print(Back.CYAN + Fore.BLACK + convertByteArrayToString(dataArray) + Style.RESET_ALL)
